...
def write_complex_pdb(receptor_pdbqt: Path, ligand_pdbqt: Path,
    ...
    """
    Merge cleaned protein (ATOM) and best-pose ligand (HETATM) into one PDB.
    Chain A = protein, Chain L = ligand.
    Viewable directly in PyMOL / UCSF ChimeraX / VMD.
    ...
    lines = [
        ...
        "REMARK   5 Chain A = Protein  |  Chain L = Ligand",
    ...
    for line in receptor_pdbqt.read_text().splitlines():
        if line.startswith("ATOM") or line.startswith("HETATM"):
            ...
            elem = ad4[:2].strip()
            col11_54 = line[11:54] if len(line) >= 54 else line[11:]
            pdb_line = "ATOM  {:5d}{:<43s}  1.00 20.00          {:>2s}  ".format(
                idx, col11_54, elem)
    ...
    for line in ligand_pdbqt.read_text().splitlines():
        if line.startswith("ATOM") or line.startswith("HETATM"):
            ...
            ad4    = line[77:79].strip() if len(line) >= 79 else "C"
            elem   = ad4[:2].strip()
            pdb_line = "HETATM{:5d} {:<4s} LIG L{:4d}    {:24s}  1.00 20.00          {:>2s}  ".format(
                idx, aname.strip(), lig_serial, coords, elem)
            lines.append(pdb_line)
            idx += 1
            lig_serial += 1
